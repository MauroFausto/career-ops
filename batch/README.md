...
4. **Results** are automatically merged into `data/applications.md` and verified with `src/pipeline/verify-pipeline.mjs` at the end of the run.
...
4. After all workers finish, batch-runner calls `src/pipeline/merge-tracker.mjs` to merge TSVs into `data/applications.md` and runs `src/pipeline/verify-pipeline.mjs` to check integrity.

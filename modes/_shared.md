...
1b. **First evaluation of each session:** Run `node src/pipeline/cv-sync-check.mjs`. If warnings, notify user.
...
`src/generators/generate-pdf.mjs` automatically normalizes em-dashes, smart quotes, and zero-width characters to ASCII equivalents for maximum ATS compatibility. But avoid generating them in the first place.

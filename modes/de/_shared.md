...
1b. **Bei der ersten Bewertung jeder Session:** `node src/pipeline/cv-sync-check.mjs` per Bash ausführen. Bei Warnungen den Kandidaten informieren, bevor weitergearbeitet wird
...
9. **Tracker-Einträge als TSV** — `applications.md` NIEMALS direkt für neue Einträge editieren. TSV in `batch/tracker-additions/` schreiben, `src/pipeline/merge-tracker.mjs` übernimmt das Mergen

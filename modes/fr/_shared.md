...
1b. **Premiere evaluation de chaque session :** Lancer `node src/pipeline/cv-sync-check.mjs` via Bash. En cas d'alertes, prevenir le candidat
...
9. **Entrees tracker en TSV** -- NE JAMAIS editer applications.md directement pour de nouveaux ajouts. Ecrire le TSV dans `batch/tracker-additions/`, `src/pipeline/merge-tracker.mjs` gere la fusion

...
1b. **各セッションの最初の評価で：** Bash で `node src/pipeline/cv-sync-check.mjs` を実行。警告があれば続行前に候補者に知らせる
...
9. **tracker エントリは TSV で** — `applications.md` を新規エントリのために直接編集しない。TSV を `batch/tracker-additions/` に書き、`src/pipeline/merge-tracker.mjs` が merge を処理する

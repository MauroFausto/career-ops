...
1b. **Na primeira avaliação de cada sessão:** Executar `node src/pipeline/cv-sync-check.mjs` via Bash. Se houver avisos, informar o candidato antes de continuar
...
9. **Entradas no tracker como TSV** — NUNCA editar `applications.md` diretamente para novos registros. Escrever TSV em `batch/tracker-additions/`, `src/pipeline/merge-tracker.mjs` cuida do merge
...
| Bash | `node src/generators/generate-pdf.mjs`, `node src/pipeline/merge-tracker.mjs` |

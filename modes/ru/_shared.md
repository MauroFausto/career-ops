...
1b. **Первая оценка в сессии:** Запустить `node src/pipeline/cv-sync-check.mjs`. Если предупреждения — уведомить пользователя.
...
`src/generators/generate-pdf.mjs` автоматически нормализует em-dash, умные кавычки и zero-width символы в ASCII для максимальной ATS-совместимости. Но лучше не генерировать их изначально.

...
Run `node src/system/update-system.mjs check` and parse the JSON output.
...
- If `dismissed`: Tell the user "Update check was previously dismissed. Clearing the dismissal and re-checking now." Remove `.update-dismissed`, then re-run `node src/system/update-system.mjs check` and branch on the new status.
...
```bash
git diff HEAD..FETCH_HEAD --stat -- modes/ CLAUDE.md AGENTS.md src/ *.mjs batch/ dashboard/ templates/ docs/ VERSION DATA_CONTRACT.md
```
...
2. Run `node src/system/update-system.mjs apply`
...
3. Run `node src/system/doctor.mjs` to validate the installation
...
5. Show final status:
   > "✅ Updated to v{version}. Run `node src/system/doctor.mjs` anytime to verify setup."

If no:
1. Run `node src/system/update-system.mjs dismiss`
...
1. Run `node src/system/update-system.mjs rollback`
...
- If anything goes wrong, tell the user to run `node src/system/update-system.mjs rollback`

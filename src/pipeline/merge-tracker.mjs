...
function validateStatus(status) {
  ...
  // Aliases ES (legado) + PT-BR → canônico EN
  const aliases = {
    'evaluada': 'Evaluated', 'avaliada': 'Evaluated', 'condicional': 'Evaluated', 'hold': 'Evaluated',
    'evaluar': 'Evaluated', 'avaliar': 'Evaluated', 'verificar': 'Evaluated',
    'aplicado': 'Applied', 'candidatado': 'Applied', 'enviada': 'Applied', 'aplicada': 'Applied', 'candidatada': 'Applied',
    'applied': 'Applied', 'sent': 'Applied',
    'respondido': 'Responded',
    'entrevista': 'Interview',
    'oferta': 'Offer',
    'rechazado': 'Rejected', 'rechazada': 'Rejected', 'rejeitado': 'Rejected', 'rejeitada': 'Rejected',
    'descartado': 'Discarded', 'descartada': 'Discarded', 'cerrada': 'Discarded', 'fechada': 'Discarded', 'cancelada': 'Discarded',
    'no aplicar': 'SKIP', 'no_aplicar': 'SKIP', 'nao candidatar': 'SKIP', 'não candidatar': 'SKIP', 'nao_candidatar': 'SKIP',
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const statusToken = 'evaluated|applied|responded|interview|offer|rejected|discarded|skip|evaluada|avaliada|aplicado|candidatado|respondido|entrevista|oferta|rechazado|rejeitado|descartado|no aplicar|nao candidatar|cerrada|fechada|duplicado|repost|condicional|hold|monitor';
    const col4LooksLikeStatus = new RegExp(`^(${statusToken})`, 'i').test(col4);
    const col5LooksLikeStatus = new RegExp(`^(${statusToken})`, 'i').test(col5);
...
if (VERIFY && !DRY_RUN) {
  ...
  try {
    execFileSync('node', [join(ROOT, 'src/pipeline/verify-pipeline.mjs')], { stdio: 'inherit' });
  } catch (e) {
    process.exit(1);
  }
}

 ...
 *   Avaliada, Candidatado, Respondido, Entrevista, Oferta, Rejeitado, Descartado, NÃO CANDIDATAR
 *
...
function normalizeStatus(raw) {
  ...
  // FECHADA / CERRADA / Cancelada / Descartada → Discarded
  if (/^fechada$/i.test(s)) return { status: 'Discarded' };
  ...
  // Rejeitada / Rejeitado / Rechazada / Rechazado → Rejected
  if (/^rejeitada?$/i.test(s)) return { status: 'Rejected' };
  if (/^rechazada?$/i.test(s)) return { status: 'Rejected' };
  if (/^rejeitado\s+\d{4}/i.test(s)) return { status: 'Rejected' };
  if (/^rechazado\s+\d{4}/i.test(s)) return { status: 'Rejected' };

  // Candidatado / Aplicado with date → Applied (strip date)
  if (/^(candidatado|aplicado)\s+\d{4}/i.test(s)) return { status: 'Applied' };

  // CONDICIONAL / HOLD / AVALIAR / EVALUAR / Verificar → Evaluated
  if (/^(condicional|hold|avaliar|evaluar|verificar)$/i.test(s)) return { status: 'Evaluated' };
  ...
  // Aliases ES (legado) + PT-BR → canônicos EN
  if (['evaluada', 'avaliada'].includes(lower)) return { status: 'Evaluated' };
  if (['aplicado', 'candidatado', 'enviada', 'aplicada', 'candidatada', 'applied', 'sent'].includes(lower)) return { status: 'Applied' };
  ...
  if (['cerrada', 'fechada', 'descartada'].includes(lower)) return { status: 'Discarded' };
  if (['no aplicar', 'no_aplicar', 'nao candidatar', 'não candidatar', 'nao_candidatar', 'skip'].includes(lower)) return { status: 'SKIP' };
...
for (let i = 0; i < lines.length; i++) {
  ...
  // Format: ['', '#', 'data', 'empresa', 'cargo', 'score', 'STATUS', 'pdf', 'report', 'notas', '']

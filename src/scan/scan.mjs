...
const PENDING_MARKERS = ['## Pendentes', '## Pendientes', '## Pending'];
const PROCESSED_MARKERS = ['## Processadas', '## Procesadas', '## Processed'];
const DEFAULT_PENDING = '## Pendentes';

function findSectionMarker(text, markers) {
...
function appendToPipeline(offers) {
  ...
  // Localizar seção de pendentes (PT-BR, ES legado ou EN)
  const pending = findSectionMarker(text, PENDING_MARKERS);
  if (!pending) {
    const processed = findSectionMarker(text, PROCESSED_MARKERS);
...
async function verifyOffers(offers) {
  ...
  try {
    ...
    ({ checkUrlLiveness } = await import('../liveness/liveness-browser.mjs'));
  } catch (err) {
    throw new Error(

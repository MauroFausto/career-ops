...
function findMjsFiles(dir) {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) results.push(...findMjsFiles(full));
    else if (entry.name.endsWith('.mjs')) results.push(full);
  }
  return results;
}

const mjsFiles = findMjsFiles(join(ROOT, 'src'));
for (const f of mjsFiles) {
  const result = run(NODE, ['--check', f]);
  if (result !== null) {
    pass(`${f.replace(ROOT + '/', '')} syntax OK`);
  } else {
    fail(`${f.replace(ROOT + '/', '')} has syntax errors`);
  }
}
...
const scripts = [
  { name: 'src/pipeline/cv-sync-check.mjs', expectExit: 1, allowFail: true },
  { name: 'src/pipeline/verify-pipeline.mjs', expectExit: 0 },
  { name: 'src/pipeline/normalize-statuses.mjs', expectExit: 0 },
  { name: 'src/pipeline/dedup-tracker.mjs', expectExit: 0 },
  { name: 'src/pipeline/merge-tracker.mjs', expectExit: 0 },
  { name: 'src/analysis/analyze-patterns.mjs --self-test', expectExit: 0 },
  { name: 'src/system/update-system.mjs check', expectExit: 0 },
];
...
try {
  const { classifyLiveness } = await import(pathToFileURL(join(ROOT, 'src/liveness/liveness-core.mjs')).href);
...
const allowedFiles = [
  ...
  'package.json', '.github/FUNDING.yml', 'CLAUDE.md', 'AGENTS.md', 'go.mod', 'src/tests/test-all.mjs',
...
const absPathResult = run(
  `git grep -n "/Users/" -- '*.mjs' '*.sh' '*.md' '*.go' '*.yml' 2>/dev/null | grep -v README.md | grep -v LICENSE | grep -v CLAUDE.md | grep -v src/tests/test-all.mjs`
);
if (!absPathResult) {
...
if (fileExists('src/scan/providers/local-parser.mjs')) {
...
if (!fileExists('src/parsers/cohere_jobs.py')) {
...
if (
  ...
  portalExample.includes('src/parsers/example-js-company-jobs.js') &&
  portalExample.includes('src/parsers/example_python_company_jobs.py') &&
...
try {
  const { buildLocationFilter } = await import(pathToFileURL(join(ROOT, 'src/scan/scan.mjs')).href);
...
try {
  const cadence = await import(pathToFileURL(join(ROOT, 'src/analysis/followup-cadence.mjs')).href);
  ...
  let cliOut = '';
  try {
    cliOut = execFileSync(NODE, [join(ROOT, 'src/analysis/followup-cadence.mjs')], { cwd: ROOT, encoding: 'utf-8', timeout: 30000 });
  } catch (cliErr) {
...
try {
  const workable = (await import(pathToFileURL(join(ROOT, 'src/scan/providers/workable.mjs')).href)).default;
  const { parseWorkableMarkdown } = await import(pathToFileURL(join(ROOT, 'src/scan/providers/workable.mjs')).href);
...
try {
  const sr = (await import(pathToFileURL(join(ROOT, 'src/scan/providers/smartrecruiters.mjs')).href)).default;
  const { parseSmartRecruitersResponse } = await import(pathToFileURL(join(ROOT, 'src/scan/providers/smartrecruiters.mjs')).href);
...
try {
  const recruitee = (await import(pathToFileURL(join(ROOT, 'src/scan/providers/recruitee.mjs')).href)).default;
  const { parseRecruiteeResponse } = await import(pathToFileURL(join(ROOT, 'src/scan/providers/recruitee.mjs')).href);
